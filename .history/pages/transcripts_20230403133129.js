...
let username = store.name;

//ref name
function TranscriptsUser(){
...
//ref name
    var messages = [];
   // await dispatch(setUser(User.name));
    await database()
    .ref(`/users/${username}/transcripts/${store.transcript}/messages`)
        .once("value") 
        .then((snapshot) => {
...
async function DeleteMessage(message, store) {
    await database()
        .ref(`/users/${username}/transcripts/${store.transcript}/messages/${message.time}`)
        .remove()
